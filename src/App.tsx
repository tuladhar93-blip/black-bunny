...
const FALLBACK_PRODUCTS = RAW_PRODUCTS.map((p, i) => {
  ...
  return {
    ...
    }piece crafted from lightweight ${fabric.toLowerCase()}, designed to move easily from day into evening. Part of the ${
      collection || 'core'
    } collection.`,
    details: [
    ...
    .map((r, i) => {
      ...
      const sizes = obj.sizes
        ? obj.sizes
            ...
            .filter(Boolean)
        : ['One Size'];
      const colorNames = obj.colorNames
        ? obj.colorNames
            .split(';')
            .map((s) => s.trim())
            .filter(Boolean)
        : ['Default'];
      const colorHexes = obj.colorHexes
        ? obj.colorHexes
            .split(';')
            .map((s) => s.trim())
            .filter(Boolean)
        : [JEWEL[i % JEWEL.length]];
      const colors = colorNames.map((n, idx) => ({
        n,
        h: colorHexes[idx] || colorHexes[0],
      }));
      const slug = (obj.name || `product-${i}`)
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/(^-|-$)/g, '');
      return {
        ...
        description: `A fluid piece crafted from lightweight ${(
          obj.fabric || 'fabric'
        ).toLowerCase()}, part of the ${obj.collection || 'core'} collection.`,
        details: [
          'Relaxed, fluid silhouette',
          `${
            obj.fabric || 'Premium fabric'
          } — sourced from long-term mill partners`,
...
function GarmentArt({ hue, altHue, hovered, pattern = 0 }) {
  ...
  return (
    ...
    >
      ...
      {/* kimono silhouette */}
      <g
        transform={hovered ? 'translate(4,0) rotate(1 200 260)' : ''}
        opacity="0.9"
      >
        <path
          d="M200 60 L150 100 L120 480 L170 500 L200 320 L230 500 L280 480 L250 100 Z"
          fill={hue}
          opacity="0.55"
        />
        <path
          d="M150 100 L60 160 L80 260 L130 220 Z"
          fill={hue}
          opacity="0.4"
        />
        <path
          d="M250 100 L340 160 L320 260 L270 220 Z"
          fill={hue}
          opacity="0.4"
        />
        <line
  ...
  return (
    <div
      className={`group cursor-pointer flex-shrink-0 ${
        size === 'carousel' ? 'w-[78%] sm:w-[46%] md:w-[280px]' : 'w-full'
      }`}
    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden bg-[#F1E9DC]">
        ...
        >
          <Heart
            size={15}
            fill={wishlist.includes(p.id) ? C.red : 'none'}
            color={wishlist.includes(p.id) ? C.red : C.black}
          />
        </button>
        <div className="absolute top-3 left-3 flex flex-col gap-1">
          {p.newArrival && (
            <span className="bg-black text-white text-[10px] tracking-widest px-2 py-1">
              NEW
            </span>
          )}
          {p.limitedEdition && (
            <span className="bg-[#8C4A45] text-white text-[10px] tracking-widest px-2 py-1">
              LIMITED
            </span>
          )}
        </div>
...
function Carousel({ title, products, onOpen, wishlist, toggleWish, viewAll }) {
  ...
  return (
    <section className="px-5 md:px-10 py-16">
      <div className="flex items-end justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          {viewAll && (
            <button
              onClick={viewAll}
              className="text-[12px] tracking-widest underline underline-offset-4"
            >
              VIEW ALL
            </button>
          )}
          <div className="hidden md:flex gap-2">
            <button
              onClick={() => scroll(-1)}
              className="w-9 h-9 border border-black/15 rounded-full flex items-center justify-center hover:bg-black hover:text-white transition"
            >
              <ChevronLeft size={16} />
            </button>
            <button
      ...
      <div
        ref={ref}
        className="flex gap-5 overflow-x-auto scrollbar-hide snap-x pb-2"
      >
        {products.map((p) => (
...
function MegaMenu({ nav, goCollection }) {
  ...
  return (
    <div className="absolute left-0 right-0 top-full bg-white border-t border-black/10 shadow-xl grid grid-cols-4 gap-8 px-14 py-10 z-40">
      <div>
        <p className="text-[11px] tracking-widest text-[#8a8378] mb-4">
          SHOP BY STYLE
        </p>
        <ul className="space-y-2.5">
          {KIMONO_STYLES.map((s) => (
            <li key={s}>
              <button
                onClick={() => goCollection('Kimonos', { style: s })}
                className="text-[13px] hover:text-[#8C4A45] transition"
              >
                {s}
              </button>
            </li>
      ...
      <div>
        ...
        <ul className="space-y-2.5">
          {OCCASIONS.map((o) => (
            <li key={o}>
              <button
                onClick={() => goCollection('Kimonos', { occasion: o })}
                className="text-[13px] hover:text-[#8C4A45] transition"
              >
                {o}
              </button>
      ...
      <div>
        ...
        <ul className="space-y-2.5">
          {[...COLLECTIONS, 'Summer 26'].map((cn) => (
            <li key={cn}>
              <button
                onClick={() => goCollection('Kimonos', { collection: cn })}
                className="text-[13px] hover:text-[#8C4A45] transition"
              >
                {cn}
              </button>
        ...
        <p className="text-[11px] tracking-widest text-[#8a8378] mt-6 mb-4">
          FEATURED
        </p>
        <ul className="space-y-2.5">
          <li>
            <button
              onClick={() => goCollection('Kimonos', { featured: 'new' })}
              className="text-[13px] hover:text-[#8C4A45] transition"
            >
              New Arrivals
            </button>
          ...
          <li>
            <button
              onClick={() => goCollection('Kimonos', { featured: 'best' })}
              className="text-[13px] hover:text-[#8C4A45] transition"
            >
              Best Sellers
            </button>
          ...
          <li>
            <button
              onClick={() => goCollection('Kimonos', { featured: 'limited' })}
              className="text-[13px] hover:text-[#8C4A45] transition"
            >
              Limited Edition
            </button>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="aspect-[3/4] overflow-hidden">
          <GarmentArt hue={C.red} altHue={C.brown} hovered={false} />
        </div>
        <div className="aspect-[3/4] overflow-hidden">
          <GarmentArt hue={JEWEL[1]} altHue={JEWEL[4]} hovered={false} />
        </div>
      </div>
    </div>
  );
...
function Header({
  ...
  return (
    ...
    >
      <div
        className="relative flex items-center justify-between px-5 md:px-10 h-[70px]"
      ...
      >
          ...
          {NAV.map((n) => (
            <button
              ...
              onClick={() =>
                n.cat
                  ? goCollection(n.cat, {})
                  : goCollection(null, { featured: 'new' })
              }
        ...
        <button
          onClick={goHome}
          className="absolute left-1/2 -translate-x-1/2 text-xl tracking-[0.15em]"
          style={{ fontFamily: "'Playfair Display', serif" }}
        >
          BLACK BUNNY
        </button>
        <div className="flex items-center gap-5">
          ...
          <button
            onClick={goAdmin}
            className="hidden md:block text-[10px] tracking-widest text-[#8a8378] hover:text-black transition"
          ...
          <button onClick={openCart} className="relative" aria-label="Bag">
            <ShoppingBag size={19} />
            {cartCount > 0 && (
              <span className="absolute -top-2 -right-2 bg-[#8C4A45] text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center">
                {cartCount}
              </span>
            )}
          </button>
...
function MobileMenu({ open, onClose, goCollection, goAdmin }) {
  return (
    ...
    >
      ...
      />
      <div
        className={`absolute top-0 left-0 bottom-0 w-[82%] max-w-[340px] bg-white transition-transform duration-300 ${
        ...
        <div className="p-6 flex flex-col gap-1">
          {NAV.map((n) => (
            <button
              ...
              className="text-left py-3.5 text-[15px] tracking-wide border-b border-black/5"
            >
              {n.label}
            </button>
          ))}
          <button
            ...
            className="text-left py-3.5 text-[12px] tracking-widest text-[#8a8378] mt-4"
          >
            STORE ADMIN
          </button>
        </div>
  ...
  return (
    <section className="relative h-[92vh] min-h-[560px] bg-[#EFE6D6] overflow-hidden flex items-end">
      ...
      <div className="relative z-10 px-6 md:px-14 pb-16 max-w-2xl">
        <p className="text-white/80 text-[12px] tracking-[0.2em] mb-4">
          WEAR THE ART.
        </p>
        <h1
          className="text-white text-[2.6rem] md:text-6xl leading-[1.02]"
          style={{ fontFamily: "'Playfair Display', serif" }}
        >
          The art of movement.
        </h1>
        <p className="text-white/85 mt-5 max-w-md text-[15px] leading-relaxed">
          Modern kimonos designed to move with you — reimagined through fluid
          silhouettes and considered fabrics.
        </p>
        <div className="flex gap-4 mt-8 flex-wrap">
          <button
            onClick={() => goCollection(null, { featured: 'new' })}
            className="bg-white text-black px-7 py-3.5 text-[12px] tracking-widest hover:bg-transparent hover:text-white border border-white transition"
          >
            SHOP NEW ARRIVALS
          </button>
          <button
            onClick={() => goCollection('Kimonos', {})}
            className="border border-white text-white px-7 py-3.5 text-[12px] tracking-widest hover:bg-white hover:text-black transition"
          >
            EXPLORE KIMONOS
          </button>
        </div>
    ...
    <section className="px-5 md:px-10 py-16">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {tiles.map((t, i) => (
          <button
            key={t.label}
            onClick={() => goCollection(t.cat || 'Kimonos', t.key)}
            className="group relative aspect-[3/4] overflow-hidden"
          >
            <div className="w-full h-full transition-transform duration-700 group-hover:scale-110">
              <GarmentArt
                hue={JEWEL[i % JEWEL.length]}
                altHue={JEWEL[(i + 2) % JEWEL.length]}
                hovered={false}
              />
            </div>
            <div className="absolute inset-0 bg-black/25 group-hover:bg-black/35 transition" />
            <span className="absolute bottom-5 left-5 text-white text-[13px] tracking-widest">
              {t.label}
            </span>
          </button>
        ))}
    ...
    <section className="relative h-[70vh] min-h-[420px] overflow-hidden">
      ...
      <div className="absolute inset-0 bg-black/45 flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-white/85 mb-7">
          Fluid silhouettes. Statement prints. Effortless movement.
        </p>
        <button
          onClick={() => goCollection('Kimonos', {})}
          className="bg-white text-black px-7 py-3.5 text-[12px] tracking-widest hover:bg-transparent hover:text-white border border-white transition"
    ...
    <section className="px-5 md:px-10 py-16 bg-[#F7F3EC]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {OCCASIONS.map((o, i) => (
          <button
            key={o}
            onClick={() => goCollection(null, { occasion: o })}
            className="group relative aspect-[16/10] overflow-hidden"
          >
            <div className="w-full h-full transition-transform duration-700 group-hover:scale-110">
              <GarmentArt
                hue={JEWEL[(i + 4) % JEWEL.length]}
                altHue={JEWEL[(i + 1) % JEWEL.length]}
                hovered={false}
              />
            </div>
  ...
  return (
    <section className="grid md:grid-cols-2">
      <div className="h-[420px] md:h-auto">
        <GarmentArt hue={C.red} altHue={JEWEL[2]} hovered={false} pattern={1} />
      </div>
      <div className="flex flex-col justify-center px-8 md:px-16 py-16 bg-[#0E0D0C] text-white">
        <p className="text-[11px] tracking-[0.2em] text-white/60 mb-4">
          FEATURED COLLECTION
        </p>
        <h2
        ...
        <p
          className="text-white/75 mb-8 max-w-sm italic"
          style={{ fontFamily: "'Playfair Display', serif" }}
        >
          Inspired by fleeting blooms and quiet moments.
        </p>
        <button
          onClick={() => goCollection(null, { collection: 'Hana' })}
          className="w-fit border border-white px-7 py-3.5 text-[12px] tracking-widest hover:bg-white hover:text-black transition"
        >
          SHOP HANA
        </button>
      </div>
  ...
  return (
    <section className="px-5 md:px-10 py-16">
      <h2
        ...
        style={{ fontFamily: "'Playfair Display', serif" }}
      >
        The Black Bunny Edit
      </h2>
      <p className="text-[#8a8378] mb-8 max-w-md">
        Five ways to wear the kimono, from morning to midnight.
      </p>
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {looks.map((l, i) => (
          <div
            key={l}
            className={`relative aspect-[3/4.4] overflow-hidden ${
              i === 2 ? 'md:mt-10' : ''
            }`}
          >
            <GarmentArt
              hue={JEWEL[(i + 5) % JEWEL.length]}
              altHue={JEWEL[i % JEWEL.length]}
              hovered={false}
            />
            <span className="absolute bottom-4 left-4 text-white text-[11px] tracking-wide max-w-[80%]">
              {l}
            </span>
          </div>
        ))}
      ...
      <button
        onClick={() => goCollection(null, {})}
        className="mt-8 text-[12px] tracking-widest underline underline-offset-4"
      >
        SHOP THE EDIT
      </button>
    </section>
...
function BrandStory() {
  return (
    <section className="px-5 md:px-10 py-24 text-center max-w-2xl mx-auto">
      <p className="text-[11px] tracking-[0.2em] text-[#8a8378] mb-5">
        OUR STORY
      </p>
      <h2
      ...
      <p className="text-[#5A4632] leading-relaxed mb-8">
        Black Bunny takes inspiration from the timeless form of the kimono and
        reinterprets it through modern fabrics, relaxed silhouettes, and
        contemporary styling.
      </p>
      <button className="text-[12px] tracking-widest underline underline-offset-4">
        OUR STORY
      </button>
    </section>
  );
    ...
    <section className="px-5 md:px-10 py-16 bg-[#F7F3EC]">
      ...
      <div className="flex gap-3 overflow-x-auto scrollbar-hide">
        {Array.from({ length: 7 }).map((_, i) => (
          <div
            key={i}
            className="w-[160px] h-[200px] flex-shrink-0 overflow-hidden"
          >
            <GarmentArt
              hue={JEWEL[i % JEWEL.length]}
              altHue={JEWEL[(i + 3) % JEWEL.length]}
  ...
  return (
    <section className="px-5 md:px-10 py-20 bg-[#0E0D0C] text-white text-center">
      <h2
        ...
        style={{ fontFamily: "'Playfair Display', serif" }}
      >
        Enter the world of Black Bunny
      </h2>
      <p className="text-white/65 mb-8 max-w-md mx-auto">
        New collections, private releases, and styling inspiration — delivered
        first.
      </p>
      {done ? (
        ...
        <form
          ...
          className="flex max-w-sm mx-auto border-b border-white/40"
        >
          <input
            required
            type="email"
            ...
            className="bg-transparent flex-1 py-3 text-[13px] outline-none placeholder:text-white/40"
          />
          <button type="submit" className="text-[12px] tracking-widest px-2">
            JOIN
          </button>
        </form>
      )}
...
function CollectionPage({
  ...
  const filtered = useMemo(() => {
    let list = base.filter((p) => {
      ...
      if (
        ...
        return false;
      if (filters.color && !p.colors.some((c) => c.n === filters.color))
        return false;
    ...
    if (sort === 'Newest')
      list = [...list].sort((a, b) => b.newArrival - a.newArrival);
    if (sort === 'Best Selling')
      list = [...list].sort((a, b) => b.bestSeller - a.bestSeller);
    return list;
  }, [base, filters, sort]);

  const allColors = [
    ...new Set(products.flatMap((p) => p.colors.map((c) => c.n))),
  ];
  ...
  const FilterGroup = ({ label, options, field }) => (
    <div className="mb-7">
      <p className="text-[11px] tracking-widest text-[#8a8378] mb-3">
        {label.toUpperCase()}
      </p>
        ...
        {options.map((o) => (
          <button
            ...
            onClick={() =>
              setFilters((f) => ({
                ...f,
                [field]: f[field] === o ? undefined : o,
              }))
            }
            className={`text-left text-[13px] flex items-center gap-2 ${
              filters[field] === o ? 'text-[#8C4A45]' : ''
            }`}
          >
            <span
              className={`w-3.5 h-3.5 border flex items-center justify-center ${
                filters[field] === o
                  ? 'bg-[#8C4A45] border-[#8C4A45]'
                  : 'border-black/25'
              }`}
            >
  ...
  return (
    <div className="px-5 md:px-10 py-10">
      ...
      <p className="text-[#8a8378] max-w-lg mb-8">
        Statement layers designed for everyday dressing, travel, and special
        moments.
      </p>
          ...
          {KIMONO_STYLES.map((s) => (
            <button
              ...
              onClick={() =>
                setFilters((f) => ({
                  ...f,
                  style: f.style === s ? undefined : s,
                }))
              }
              className={`flex-shrink-0 px-4 py-2 text-[11px] tracking-widest border ${
                filters.style === s
      ...
      <div className="flex items-center justify-between border-y border-black/10 py-3 mb-8">
        <button
          ...
          className="flex items-center gap-2 text-[12px] tracking-widest"
        >
          <SlidersHorizontal size={14} /> FILTER
        </button>
        <span className="text-[12px] text-[#8a8378]">
          {filtered.length} PRODUCTS
        </span>
        <select
          value={sort}
          onChange={(e) => setSort(e.target.value)}
          className="text-[12px] tracking-widest bg-transparent outline-none"
        >
          {[
            'Featured',
            'Newest',
      ...
      {filtered.length === 0 ? (
        <p className="text-center py-20 text-[#8a8378]">
      ...
      >
        ...
        />
        <div
          className={`absolute top-0 left-0 bottom-0 w-[85%] max-w-[320px] bg-white p-6 overflow-y-auto transition-transform duration-300 ${
        ...
        >
          ...
          <button
            onClick={() => setFilters({})}
            className="text-[12px] underline underline-offset-4 mt-2"
          >
            Clear all filters
          </button>
        </div>
...
function ProductPage({
  ...
  const related = products
    ...
    .slice(0, 4);
  const alsoLove = products
    .filter((x) => x.category === p.category && x.id !== p.id)
    .slice(0, 4);

  const handleAdd = () => {
    if (p.sizes[0] !== 'One Size' && !size) {
      addToast('Select a size first');
      return;
    }
  ...
  const Accordion = ({ title, content }) => (
    <div className="border-b border-black/10">
      <button
        ...
        className="w-full flex justify-between items-center py-4 text-[13px] tracking-wide"
      >
        {title.toUpperCase()}
        <ChevronDown
          size={15}
      ...
      {openSection === title && (
        <div className="pb-4 text-[13px] text-[#5A4632] leading-relaxed">
          {content}
        </div>
      )}
    </div>
    ...
    <div className="px-5 md:px-10 py-10">
      <div className="grid md:grid-cols-2 gap-10">
            ...
            <div className="flex gap-2">
              {shots.map((s, i) => (
                <button
                  key={s}
                  onClick={() => setImgIdx(i)}
                  className={`flex-1 aspect-square overflow-hidden border-2 ${
        ...
        <div className="md:pt-2">
          <p className="text-[11px] tracking-widest text-[#8a8378] mb-2">
            {p.collection
          ...
          <div className="flex items-center gap-1.5 mb-6 text-[13px]">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star
                key={i}
                size={13}
                fill={i < Math.round(p.rating) ? C.black : 'none'}
                color={C.black}
              />
            ))}
            <span className="text-[#8a8378] ml-1">
            ...
            {p.colors.map((c) => (
              <button
                ...
                className={`w-9 h-9 rounded-full border-2 ${
                  color === c.n ? 'border-black' : 'border-transparent'
                }`}
                style={{
                  background: c.h,
          ...
          {p.sizes[0] !== 'One Size' && (
            <>
              <div className="flex justify-between items-center mb-2">
                ...
                <button className="text-[11px] underline underline-offset-4 text-[#8a8378]">
                  Size Guide
                </button>
              </div>
              <div className="flex gap-2 mb-6">
                {p.sizes.map((s) => (
                  <button
                    key={s}
                    onClick={() => setSize(s)}
                    className={`w-11 h-11 border text-[12px] ${
                      size === s
                        ? 'bg-black text-white border-black'
                        : 'border-black/20'
                    }`}
          ...
          <div className="flex gap-3 mb-8">
            <button
              onClick={handleAdd}
              className="flex-1 bg-black text-white py-4 text-[12px] tracking-widest hover:bg-[#8C4A45] transition"
            >
              ADD TO BAG
            </button>
            <button
              onClick={() => toggleWish(p.id)}
              className="w-14 border border-black/20 flex items-center justify-center"
            >
              <Heart
                size={18}
                fill={wishlist.includes(p.id) ? C.red : 'none'}
          ...
          <Accordion
            title="Fabric & Care"
            content={`${p.fabric}. Hand wash cold or dry clean. Hang to dry away from direct sun. Cool iron if needed.`}
          />
          <Accordion
            title="Shipping & Returns"
            content="Complimentary shipping on orders Rs 15,000+. Free returns within 30 days of delivery."
          />
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-10">
          {items.map((p) => (
            <div key={p.id}>
              ...
              <button
                onClick={() => {
                  addToCart(p, p.sizes[0], p.colors[0].n);
                  addToast('Added to bag');
                }}
                className="mt-2 w-full border border-black/20 py-2.5 text-[11px] tracking-widest hover:bg-black hover:text-white transition"
              >
                ADD TO BAG
              </button>
            </div>
...
function CartDrawer({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`absolute top-0 right-0 bottom-0 w-[420px] max-w-[94vw] bg-white flex flex-col transition-transform duration-300 ${
          open ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        ...
        <div className="px-6 py-3 border-b border-black/10">
          ...
          <div className="h-1 bg-black/10 rounded-full overflow-hidden">
            <div
              className="h-full bg-[#8C4A45] transition-all"
              style={{ width: `${pct}%` }}
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6">
          {cart.length === 0 ? (
            <p className="text-center py-20 text-[#8a8378] text-[13px]">
              Your bag is empty.
            </p>
          ) : (
            cart.map((item, idx) => (
              <div
                key={idx}
                className="flex gap-4 py-5 border-b border-black/5"
              >
                <div className="w-20 h-24 flex-shrink-0 overflow-hidden">
                  <ProductImage p={item} hovered={false} />
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="w-6 h-6 border border-black/20 rounded-full flex items-center justify-center"
                    >
                      <Minus size={11} />
                    </button>
                    ...
                    <button
                      ...
                      className="ml-auto text-[11px] underline underline-offset-4 text-[#8a8378]"
                    >
                      Remove
                    </button>
                  </div>
        ...
        <div className="px-6 py-5 border-t border-black/10">
          ...
          <button
            onClick={goCheckoutToast}
            disabled={cart.length === 0}
            className="w-full bg-black text-white py-3.5 text-[12px] tracking-widest mb-2.5 disabled:opacity-40 hover:bg-[#8C4A45] transition"
          >
            CHECKOUT
          </button>
          <button
            ...
            className="w-full border border-black/20 py-3.5 text-[12px] tracking-widest hover:bg-black hover:text-white transition"
          >
            VIEW BAG / CONTINUE SHOPPING
          </button>
        </div>
      </div>
    </div>
...
function SearchOverlay({ products, open, onClose, onOpen }) {
  ...
  return (
    <div
      className={`fixed inset-0 z-[90] bg-white transition-transform duration-300 ${
        open ? 'translate-y-0' : '-translate-y-full'
      }`}
    >
      <div className="px-6 md:px-14 pt-8">
        <div className="flex justify-between items-center mb-8">
          <span className="text-[13px] tracking-widest text-[#8a8378]">
            SEARCH BLACK BUNNY
          </span>
          <button onClick={onClose}>
            <X size={22} />
        ...
        <input
          ...
          className="w-full text-2xl md:text-3xl border-b border-black/20 pb-4 outline-none"
          style={{ fontFamily: "'Playfair Display', serif" }}
        />
        ...
        {q.length <= 1 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {trending.map((t) => (
                <button
                  key={t}
                  onClick={() => setQ(t)}
                  className="border border-black/15 px-4 py-2 text-[12px] hover:bg-black hover:text-white transition"
                >
                  {t}
                </button>
              ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-8 max-h-[60vh] overflow-y-auto pb-10">
            {results.length === 0 ? (
              <p className="col-span-full text-[#8a8378] text-[13px]">
                No results for "{q}"
              </p>
            ) : (
              results.map((p) => (
                ...
                >
                  <div className="aspect-[4/5] overflow-hidden mb-2">
                    <ProductImage p={p} hovered={false} />
                  </div>
...
function Footer({ goCollection }) {
  const cols = [
    ...
    {
      h: 'ABOUT BLACK BUNNY',
      items: ['Our Story', 'Journal', 'Sustainability', 'Careers'],
    },
    {
      h: 'DISCOVER',
      items: ['New Arrivals', 'Best Sellers', 'Gift Cards', 'Lookbook'],
    },
    ...
    <footer className="bg-[#0E0D0C] text-white/70 px-5 md:px-10 pt-16 pb-8">
      <div className="grid md:grid-cols-5 gap-10 pb-12 border-b border-white/10">
        <div className="md:col-span-1">
          ...
          <p className="text-[13px] mt-4 max-w-[220px] leading-relaxed">
            Modern kimonos, reinterpreted through contemporary silhouettes and
            premium materials.
          </p>
        </div>
        {cols.map((c) => (
          <div key={c.h}>
            <p className="text-[11px] tracking-widest text-white/50 mb-4">
              {c.h}
            </p>
            <ul className="space-y-2.5">
              {c.items.map((i) => (
                <li key={i}>
                  <button className="text-[13px] hover:text-white transition text-left">
                    {i}
                  </button>
                </li>
              ))}
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-6 text-[11px]">
        <div className="flex gap-4">
          <select className="bg-transparent border border-white/20 px-3 py-1.5 text-[11px]">
...
function Toast({ msg }) {
  return (
    <div
      className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] bg-black text-white px-6 py-3 text-[13px] tracking-wide transition-all duration-300 ${
        msg
...
/* ============================== ADMIN ============================== */
function AdminPage({ products, onSave, onDelete, onBack }) {
  const emptyForm = {
    id: null,
    name: '',
    category: 'Kimonos',
  ...
  const addColor = () =>
    setForm((f) =>
      f.colors.length >= 4
        ? f
        : { ...f, colors: [...f.colors, { n: 'New Color', h: '#8C4A45' }] }
    );
  ...
  const submit = async (e) => {
    ...
    const product = {
      ...
      description:
        form.description ||
        `A fluid piece crafted from ${form.fabric || 'premium fabric'}.`,
      details: existing.details || [
        'Relaxed, fluid silhouette',
        `${form.fabric || 'Premium fabric'}`,
        'Cut and sewn in small batches',
      ],
    };
    setSaving(true);
  ...
  return (
    <div className="min-h-screen bg-[#F7F3EC] px-5 md:px-10 py-8">
      ...
      <p className="text-[12px] text-[#8a8378] mb-8 max-w-lg">
        Changes save instantly to the live site for anyone with this link.
        There's no login on this page — treat the link as private.
      </p>

      <div className="grid lg:grid-cols-[380px_1fr] gap-10">
        <form
          onSubmit={submit}
          className="bg-white p-6 space-y-5 border border-black/10 h-fit"
        >
          ...
          <div>
            ...
            <label className="relative block aspect-[4/5] bg-[#F1E9DC] border border-black/15 cursor-pointer overflow-hidden group">
              ...
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/45 transition flex items-center justify-center opacity-0 group-hover:opacity-100">
                <span className="text-white text-[11px] tracking-widest border border-white px-3 py-1.5">
            ...
            {form.image && (
              <button
                ...
                className="mt-2 text-[11px] text-[#8C4A45] underline underline-offset-4"
              >
                Remove photo
              </button>
            )}
          ...
          <div>
            ...
            <input
              ...
              className="w-full border border-black/15 px-3 py-2 text-[13px]"
            />
          </div>
              ...
              {form.colors.map((c, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input
                    type="color"
                    value={c.h}
                    onChange={(e) => updateColor(i, 'h', e.target.value)}
                    className="w-8 h-8 border border-black/15"
                  />
                  <input
                    value={c.n}
                    onChange={(e) => updateColor(i, 'n', e.target.value)}
                    className="flex-1 border border-black/15 px-2 py-1.5 text-[12px]"
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              disabled={saving}
              className="flex-1 bg-black text-white py-3 text-[12px] tracking-widest hover:bg-[#8C4A45] transition disabled:opacity-50"
            >
            ...
            {form.id && (
              <button
                type="button"
                onClick={resetForm}
                className="border border-black/20 px-4 text-[12px] tracking-widest"
              >
                CANCEL
              </button>
            )}
        ...
        <div>
          <p className="text-[12px] tracking-widest text-[#8a8378] mb-4">
            {products.length} PRODUCTS LIVE ON SITE
          </p>
          <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {products.map((p) => (
              <div
                key={p.id}
                onClick={() => startEdit(p)}
                className="bg-white border border-black/10 p-3 cursor-pointer hover:border-black/30 transition"
              >
                <div className="aspect-[4/5] overflow-hidden mb-2 bg-[#F1E9DC] relative">
                  ...
                  <div className="absolute inset-0 bg-black/0 hover:bg-black/35 transition flex items-center justify-center opacity-0 hover:opacity-100">
                    <span className="text-white text-[10px] tracking-widest border border-white px-2.5 py-1">
                      EDIT
                    </span>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 border border-black/20 py-1.5 text-[11px] tracking-widest hover:bg-black hover:text-white transition"
                  >
                    EDIT
                  </button>
                  <button
                    ...
                    className="border border-black/20 px-3 py-1.5 text-[11px] text-[#8C4A45] hover:bg-[#8C4A45] hover:text-white transition"
                  >
                    DELETE
                  </button>
                </div>
...
export default function App() {
  ...
  const persistProducts = async (list) => {
    try {
      await storage.set('bb_products', JSON.stringify(list), true);
    } catch (e) {
      addToast("Couldn't save — storage error");
    }
  };
  ...
  const goCollection = (cat, filters) => {
    setActiveCat(cat);
  ...
  return (
    <div
      className="min-h-screen bg-white text-[#0E0D0C]"
    ...
    >
      ...
      {view === 'home' && (
        <>
          <Hero goCollection={goCollection} />
          <Carousel
            title="New Arrivals"
            ...
            wishlist={wishlist}
            toggleWish={toggleWish}
            viewAll={() => goCollection(null, { featured: 'new' })}
          />
          ...
          <Carousel
            ...
            wishlist={wishlist}
            toggleWish={toggleWish}
            viewAll={() => goCollection(null, { featured: 'best' })}
          />
      ...
      {view === 'product' && (
        <ProductPage
          ...
          slug={activeSlug}
          onOpen={openProduct}
          addToCart={(p, s, c) => addToCart(p, s, c)}
          wishlist={wishlist}
          toggleWish={toggleWish}
          addToast={addToast}
        />
      ...
      <CartDrawer
        ...
        goCheckoutToast={() =>
          addToast(
            "This is a portfolio demo — checkout isn't connected to real payment."
          )
        }
        goCollection={goCollection}
      />
      ...
      {sheetStatus === 'no-url' && view !== 'admin' && (
        <div className="fixed bottom-4 right-4 z-[110] bg-amber-100 border border-amber-300 text-amber-900 text-[11px] px-4 py-2.5 max-w-[260px] rounded shadow-lg">
          Showing sample products — open{' '}
          <button onClick={goAdmin} className="underline font-medium">
            Admin
          </button>{' '}
          to add your real catalog.
        </div>
      )}
      {sheetStatus === 'error' && (
        <div className="fixed bottom-4 right-4 z-[110] bg-red-100 border border-red-300 text-red-900 text-[11px] px-4 py-2.5 max-w-[260px] rounded shadow-lg">
          Couldn't load your product sheet — showing sample products instead.
          Check that the sheet is published to the web as CSV.
        </div>
      )}
      <Toast msg={toast} />
    </div>
